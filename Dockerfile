...
RUN curl --location "https://github.com/ghostty-org/ghostty/archive/refs/tags/v1.2.0.tar.gz" -o ghostty.tar.gz
RUN tar -xvzf ghostty.tar.gz
RUN cd ghostty-1.2.0; zig build -Doptimize=ReleaseFast
RUN cd ghostty-1.2.0/zig-out; tar --zstd -c . -f /ghostty.tar.zst
